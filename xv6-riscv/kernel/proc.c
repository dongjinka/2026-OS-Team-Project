...
scheduler(void)
{
  ...
  for(;;){
    ...
    // Tiebreak on creation_tick (older creation wins). Hold best->lock
    // across the swap so the picked proc can't change state under us.
    struct proc *best = 0;
...
//
// Prepass-computes min_vruntime and floors each woken sleeper's vruntime to
// it. Without this, an I/O-bound process that slept for a long time would
// monopolize the CPU on wake (its vruntime would lag far below the running
// procs'), starving them. The brief race window between the prepass and the
// per-proc loop is acceptable; standard CFS has the same flavor.
void
wakeup(void *chan)
{
  struct proc *p;
