...
| F6 JSON 역직렬화 | ⚠️ 설계 결정 필요 | 현 설계(호스트 측 파싱) 유지·문서화 권장 (§5.1) |
...
### 5.1 F6 — JSON 역직렬화 위치 (설계 결정)

JSON 파싱은 호스트(`agent.py`)가 수행하는 현 설계가 안정성·복잡도 면에서
낫다. 다만 세중 제안 원문은 "xv6 내부 구현"을 명시하므로 둘 중 하나를 골라
기술 보고서에 명시 필요:
- **(A) 현 설계 유지 — 권장.** "JSON 파싱은 호스트, 커널은 검증된 최소 포맷만
  수신"으로 설계 근거 기재.
- **(B) 제안서 충실 이행.** `kernel/json.c` 신설, 미니 JSON 파서 + 고정 크기
  `struct json_obj`. 부동소수·동적할당 금지. 단점: 커널 복잡도·크래시 위험.
...
- `priority_test` Test 3은 완료 순서를 프로그램적으로 검증하지 않음.
  `burn()` 반복수를 키우거나 tick 점유율 측정 프로그램을 추가하면 F3/F4
  우선순위 효과가 수치로 명확해짐.
